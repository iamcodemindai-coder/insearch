...
"""
    ...
    for item in results:
        ...
        received[notif_id] = round(score, 4)
...
def run_batch(
    ...
    """
    Process every JSON file in input_dir sequentially.

    The original notification order is preserved.
    Each notification becomes one Excel row.
    Individual output JSON files are saved in output_dir.
    """
    ...
    for file_index, input_file in enumerate(json_files, start=1):
        print("\n" + "=" * 70)
        print(f"BATCH {file_index}/{len(json_files)}")
        ...
        try:
            ...
            output, llm_time = process(data)

            output_file = output_dir / f"{input_file.stem}_output.json"
            save_json(output, output_file)

            # Output is already rebuilt in ORIGINAL notification order.
            for position, item in enumerate(
                ...
                notif_id = normalize(item.get("pcm_inv_notif_id"))
                ...
                sheet.append([
                    input_file.name,
                    normalize(data.get("pcm_issue_summary")),
                    position,
                    notif_id,
                    notification_text,
                    round(score, 4),
                    round(llm_time, 3),
                    round(time.perf_counter() - case_start, 3)
                ])

            print(f"LLM response time : {llm_time:.3f} sec")
            print(
                f"Total time        : "
                f"{time.perf_counter() - case_start:.3f} sec"
            )
            print(f"Output JSON       : {output_file}")

        except Exception as e:
            # One bad input must not stop the complete batch.
    ...
    # Business-friendly Excel formatting.
    ...
    print("=" * 70)
    print(f"Cases processed : {len(json_files)}")
    print(f"Excel saved     : {excel_file}")
    print(f"Total batch time: {total_batch_time:.3f} sec")
...
def main():
    ...
    # Batch mode: if JSON files exist in batch_inputs, process all of them.
    # Otherwise, keep the original example_input.json workflow.
    batch_dir = Path("batch_inputs")

    if batch_dir.exists() and list(batch_dir.glob("*.json")):
        run_batch(
            input_dir="batch_inputs",
            excel_file="batch_results.xlsx",
            output_dir="batch_outputs"
